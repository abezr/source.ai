...
class TextSanitizer:
    ...
    def _compile_patterns(self):
        ...
        self.markdown_patterns = [
            ...
            (re.compile(r'#+\s+.*?(?=\n|$)'), 'markdown_heading_inline'),  # More flexible heading match
        ...
        self.instruction_patterns = [
            ...
            (re.compile(r'(?i)assistant\s*:.*?(?=\n\n|\n[A-Z]|\Z)'), 'assistant_override'),
            (re.compile(r'(?i)user\s*:.*?(?=\n\n|\n[A-Z]|\Z)'), 'user_override'),

            # Direct instructions
            (re.compile(r'(?i)ignore\s+(all\s+)?previous\s+instructions'), 'ignore_instructions'),
            (re.compile(r'(?i)forget\s+(all\s+)?previous\s+(instructions|rules)'), 'forget_instructions'),
            (re.compile(r'(?i)override\s+(all\s+)?previous\s+(instructions|rules)'), 'override_instructions'),
            (re.compile(r'(?i)disregard\s+(all\s+)?previous\s+(instructions|rules)'), 'disregard_instructions'),
            ...
            (re.compile(r'(?i)output\s+(only|just|exclusively)\s+(json|xml|html|markdown)'), 'output_format'),
            (re.compile(r'(?i)return\s+(only|just|exclusively)\s+(json|xml|html|markdown)'), 'return_format'),
            (re.compile(r'(?i)respond\s+(only|just|exclusively)\s+(in|with)\s+(json|xml|html|markdown)'), 'respond_format'),

            # Dangerous commands
            (re.compile(r'(?i)(run|execute|eval|system|shell|cmd|powershell)\s+.*?(?=\n|$)'), 'dangerous_command'),
            (re.compile(r'(?i)(import|require|load)\s+(os|sys|subprocess|shutil)'), 'dangerous_import'),
        ...
        self.malicious_patterns = [
            ...
            (re.compile(r'(?i)(select|insert|update|delete|drop|create|alter)\s+.*?(from|into|table|database)'), 'sql_injection'),

            # XSS attempts
            (re.compile(r'<script[^>]*>[\s\S]*?</script>', re.IGNORECASE), 'script_tag'),
            ...
            (re.compile(r'/etc/passwd|/etc/shadow|/etc/hosts', re.IGNORECASE), 'sensitive_file'),
            ...
            (re.compile(r'https?://[^\s<>"{}|\\^`\[\]]+', re.IGNORECASE), 'suspicious_url'),
        ]
        ...
        self.structural_patterns = [
            ...
            # Control characters
            (re.compile(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]'), 'control_characters'),
    ...
    def sanitize_text_for_llm(self, text: str, context: str = "general") -> SanitizationResult:
        ...
        text = self._sanitize_code_blocks(text, changes_made)  # Process code blocks first
        ...
        text = self._sanitize_malicious_patterns(text, changes_made)  # Then malicious patterns
        text = self._sanitize_structural_issues(text, changes_made)   # Finally structural issues
