...
Provides abstracted interface for interacting with multiple LLM providers.
"""
...
class LLMProvider(abc.ABC):
    ...
    def generate_structured_json(self, prompt: str) -> Dict[str, Any]:
        """
        Generate structured JSON response from a given prompt.
    ...
    def validate_connection(self) -> bool:
        """
        Validate that the LLM provider can connect to the API.
...
class GeminiProvider(LLMProvider):
    ...
    def generate_structured_json(self, prompt: str) -> Dict[str, Any]:
        """
        Generate structured JSON response from a given prompt using Gemini.
...
class OllamaProvider(LLMProvider):
    """
    Implementation of LLMProvider for Ollama local API.
    """
    ...
    def generate_structured_json(self, prompt: str) -> Dict[str, Any]:
        """
        Generate structured JSON response from a given prompt using Ollama.
        ...
        try:
            ...
            # Extract the JSON from the response
            json_text = self._extract_json_from_response(result.get("response", ""))
            return json.loads(json_text)
    ...
    def validate_connection(self) -> bool:
        """
        Validate that the Ollama provider can connect to the API.
    ...
    def _extract_json_from_response(self, response_text: str) -> str:
        """
        Extract JSON from LLM response, handling potential formatting.
...
# Legacy compatibility - deprecated, use LLMRouter instead
class LLMClient:
    """
    ...
    def __init__(self):
        """Initialize with router for dynamic provider selection."""
        # Defer router import to avoid circular import
        self._router = None
        ...
        if self.api_key:
            # Configure the API for legacy compatibility
            genai.configure(api_key=self.api_key)
            self.client = genai.GenerativeModel(self.model_name)
    ...
    def get_structured_toc(self, toc_text: str) -> Dict[str, Any]:
        """Legacy method - use router for parser role."""
        provider = self.router.get_provider_for_role("parser")
        prompt = f"""
...
"""
        return provider.generate_structured_json(prompt)
...
"""
    ...
    def generate_grounded_answer(self, query: str, context: str) -> "schemas.Answer":
        """Legacy method - use router for rag_generator role."""
        provider = self.router.get_provider_for_role("rag_generator")
        return provider.generate_grounded_answer(query, context)

    def validate_connection(self) -> bool:
        """Legacy method - check all configured providers."""
        providers = self.router.get_available_providers()
        return any(provider["connection_status"] for provider in providers)
...
def get_llm_client():
    ...
    Provides an LLMClient instance for backward compatibility.
